# Phase 4 contains export of obj and creation of materials.rad, scene.rad, scene.rif files
...
class ExportRadiance(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        direction = cam.matrix_world.to_quaternion() @ mathutils.Vector((0.0, 0.0, -1.0))
        ...
        min_x = min(bound_box, key=lambda x: x[0])[0]
        max_x = max(bound_box, key=lambda x: x[0])[0]
        min_y = min(bound_box, key=lambda x: x[1])[1]
        max_y = max(bound_box, key=lambda x: x[1])[1]
        min_z = min(bound_box, key=lambda x: x[2])[2]
        max_z = max(bound_box, key=lambda x: x[2])[2]
